import type { PerfilColor, PublicReport } from "./contract";

/** Portado de `motor/server/src/lib/telegram-map.ts`. */
...
export function telegramUpdateToPublicReport(
  ...
  const display = msg.from?.first_name ?? msg.from?.username ?? `tg:${fromId}`;
