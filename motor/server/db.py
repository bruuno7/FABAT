"""Historial SQLite construido únicamente desde el estado público de ``/api/state``."""
...
def configured_path(value: str | Path | None = None) -> Path | None:
    raw = str(value) if value is not None else os.environ.get("MANDO_DB", str(DEFAULT_PATH))
...
def _json(value: Any) -> str:
    return json.dumps(_scrub(value), ensure_ascii=False, sort_keys=True, separators=(",", ":"), default=str)
...
class Recorder:
    ...
    def offer(self, state: dict[str, Any]) -> None:
        ...
        with self._lifecycle:
            if self._closed or self.path is None or not isinstance(state, dict):
                return
            try:
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    self._queue.put_nowait(snapshot)
            except Exception as exc:  # ni una copia defectuosa detiene el reloj
                self._fail(exc)
